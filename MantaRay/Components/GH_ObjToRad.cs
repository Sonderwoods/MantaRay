    ...
    public class GH_ObjToRad : GH_Template_SaveStrings
    {
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            pManager[pManager.AddTextParameter("Target folder", "Target folder", "Target folder\n" +
                "Examples:\n" +
                "~/simulation/radFiles\n" +
                "C:\\simulation\\radFiles", GH_ParamAccess.item, "radFiles")].Optional = true;
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            {
                if (allFilePaths[i] == null) continue;

                string filePath = allFilePaths[i];
                //try
                //{
                string uploadedServerPath = sshHelper.Upload(filePath, intendedTargetFolder.ApplyGlobals(), sb);
            ...
            string ending = "<Project>".ApplyGlobals().Split('/').LastOrDefault();

            if(!string.IsNullOrEmpty(ending))
            {
                for (int i = 0; i < radFilePaths.Count; i++)
                {
                    if (radFilePaths[i].StartsWith(ending))
                    {
                        radFilePaths[i] = radFilePaths[i].Substring(ending.Length + 1);
                    }
            }
            }
